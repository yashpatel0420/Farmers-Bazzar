...
import { signInStart } from "../redux/vendor/vendor.reducer";
...
const Login = () => {
  ...
  const onLogin = (e) => {
    e.preventDefault();
    FetchAPI("/api/vendors/login", "POST", loginState).then((res) => {
      if (res.success) {
        disptach(customerSignOutStart());
        disptach(signInStart(res.vendor));
        return navigate(`/vendor/${res.vendor._id}`);
      }
      alert(res.msg);
    });
  };
